...
2. **Fund an account** — users deposit fiat via **Stripe**. The app presents deposits in **USD only** (no currency picker). The account balance is always held in **USD**. Retirement is blocked until that balance covers the marked-up cost. **No crypto deposits.** (The API may still accept EUR presentment for non-app clients; the mobile UI never offers it.)

   **Minimum deposit:** **$5.00** (500 USD cents). Keeps Stripe’s fixed per-charge fee small relative to the balance so the 40% retirement markup can cover processing costs. (Stripe’s own card floor is lower; we enforce the product minimum.)
...
**Stripe fees:** deposits credit **gross** — pay $10 → $10 available. Processing fees come out of our Stripe settlement, not the user’s balance. We recover them via the retirement markup (no deposit surcharge, no net-of-fees credit). Size `MARKUP_BPS` so it covers Klima cost + expected card/FX fees + margin.
