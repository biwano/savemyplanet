...
- Decision: set ambiguous to false only when one typical reading clearly dominates. If several plausible readings would change the order of magnitude, set ambiguous to true.
- suggestedRetirementMessage: one short sentence (max 120 characters) suitable as a public note on a retirement certificate. Rephrase the activity into a clear, dignified dedication — not a copy of the user's raw wording, not the rationale, and not tonnes or prices. Example: “Clearing emissions from my 100 km car trip.” Do not invent details the user did not imply, except ordinary defaults you also used for the estimate (state those in the rationale).
- If ambiguous is false: you may use ordinary defaults for missing but typical details (e.g. a standard commute distance). State those defaults briefly in the rationale. Do not invent travel legs or quantities the user did not imply. Rationale: one or two plain sentences explaining the estimate. No wholesale carbon-market prices. Do not ask the user to adjust, refine, or supply more detail.
- If ambiguous is true: still include a best-guess suggestedTonnes and a best-effort suggestedRetirementMessage. The rationale MUST name the concrete details the user should add (e.g. hours or distance, fuel or energy type, number of people, vehicle or cabin class, how often)—not only a ballpark figure or vague “depending on usage”. Example shape: “Roughly 0.1 tCO₂e for a typical outing; add how many hours you rode and whether it was petrol or electric.” Keep it to one or two short sentences. Never set ambiguous to false and also tell the user to adjust for accuracy.`
